...
public class CreateDocument_Test extends BaseClass {
	...
	String className = "";
	String AppendixNumber = "77";
	String Appendix = "TestCreateDocument355";
	String fileUploadPath="";
	String uploadFileName="LargeTestFile.doc";
	...
	public void Create_doc() {
		...
		sleep(2);
		test.log(LogStatus.PASS,
				"1.Click on Search by location drop-down and select one Location Ensure the document type selected does not have a template associated with it"
		...
		// is"+Credoc.getdocumentnumber().getAttribute("value"));
		 ...
		 * test.log(LogStatus.PASS,"10.Turn upper right-hand corner Edit Mode to off. "
		 * +"<br/>" +"<b> ER7: All the documents fields should get disabled..<b>"+
		...
		sleep(2);
		/*
		 * Credoc.getBrouse().sendKeys(
		 * "C:\\Users\\dell\\Desktop\\Title21data\\testing_data\\Sizeval.doc");
		 * sleep(7); Credoc.getAddButtonupload().click();
		 * //javaScriptClick(Credoc.getAddButtonupload()); sleep(6);
		 * if(Credoc.UploadFileSizeValidation()) { test.log(LogStatus.
		 * PASS,"14. Add file with size more than 50 MB  and click on Add  button  "
		 * +"<br/>"
		 * +"<b> ER6: ER 11 : It should show validation message as \"File size must be less than 50 MB\".<b>"
		 * + test.addScreenCaptpfure(captureScreenShot(driver, "File_Size")));
		 * 
		 * }
		 */
		String home_page = driver.getWindowHandle();
		//Credoc.getBrouse().sendKeys(".\\testdata\\test.doc");// TestCase-WI-Creation of new Groups
	...
	//	Credoc.getBrouse().sendKeys(fileUploadPath);
		...
		if (downloadedFile.exists()) {
			test.log(LogStatus.PASS, "15.Attached main file to the document for eg. Any doc or pdf file " + "<br/>"
					+ "16.Click on native button under the main file" + "<br/>"
					+ "<b>ER9: It should download the main file document in its native form.<b>"
		...
		Credoc.getpdf().click();
		test.log(LogStatus.PASS,
				"17. click on pdf button " + "<br/>"
						+ "<b>ER 10 : Document is converted to pdf form and open in new tab.<b>"
		...
		sleep(10);
		test.log(LogStatus.PASS,"18.Click on doc option context menu."+"<br/>"+
				"19. Go to Action section and click on Check-In.  " + "<br/>"
								+ "<b>ER 11 : Check In popup screen is displayed.<b>"
		...
		if (Credoc.CheckinSuccessmessage()) {
			test.log(LogStatus.PASS, "<b>ER 12 :  A Successful message that the document has been checked In is displayed.<b>"
							+ test.addScreenCapture(captureScreenShot(driver, "checkinsuccessmessage")));
		}
		sleep(15);
		...
		if (Credoc.editmodedisable().isDisplayed()) {
			test.log(LogStatus.PASS, "20.Click on close button " + "<br/>" + "<b>ER13: Doument Edit mode is enable.<b>"
		...
		if (Credoc.getpoupcheckin().isDisplayed())
			test.log(LogStatus.PASS,
					"21 again click onm context menu  " + "<br/>" + "22.Click on checkout link." + "<br/>"
							+ "<b> ER 14 : Check Out popup screen should get open.  <b>"
							+ test.addScreenCapture(captureScreenShot(driver, "checkinsuccessmessage")));
		sleep(2);
		...
		if (Credoc.checkoutversionvalidation()) {
			test.log(LogStatus.PASS,
					"23. Check the open document after checkout checkbox " + "<br/>" + "24.Click on confirm button"
							+ "<br/>" + "<b>ER 15 :  Document version should get changed.<b>"
							+ test.addScreenCapture(captureScreenShot(driver, "checkoutvesionvalidation")));
		}
		//sleep(5);
		sleep(15);
		if (Credoc.getEditModeOff().isDisplayed()) {
			test.log(LogStatus.PASS, "<b>ER 16 :  Document edit mode enable.<b>"
